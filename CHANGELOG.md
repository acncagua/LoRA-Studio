...
- README.mdを英語ランディングページとして再整理しました。
- README_ja.mdを日本語ランディングページとしてREADME.mdと同じ見出し構成に揃えました。
- README_ja.mdに蓄積していた詳細説明を `docs/ja/` の日本語詳細マニュアルへ分離しました。
- 分割前のREADME_ja全文を `docs/ja/_archive_readme_ja_before_split.md` に保全しました。
- `docs/ja/index.md` を追加し、日本語詳細ドキュメントの目次を整備しました。
- スクリーンショットはOSS応募向けにサニタイズした英語デモ画面であることをREADME.md / README_ja.mdに明記しました。

### 注意点

- 英語の詳細ドキュメントは今後 `docs/` 配下へ段階的に分割予定です。
- 日本語詳細ページはREADME_ja分割の初期版であり、今後画面変更に合わせて更新します。
