...
- `open_clip`: Phase 11.5以降で追加予定です。現時点ではPreflightの依存確認と設計枠のみ残しています。

Embedding設定は `Embedding設定` 画面で管理します。初期値は `mock_image_512`、cache rootは `data/embeddings`、自動downloadはOFFです。`transformers_clip` は `モデルdownloadを許可` がONの場合だけ `from_pretrained` によるdownloadを許可します。OFFのまま実行する場合は、モデルがローカルcacheに存在している必要があります。
...
`transformers_clip` providerは、画像全体の意味・構図・雰囲気を拾いやすい一方、顔専用の同一性判定ではありません。顔の細部、衣装の細かい差、キャラクター固有パーツは人間評価を優先してください。特にfull body画像は顔が小さくなるため、顔類似度として過信しないでください。Reference Setには `face_front`、`upper_body`、`full_body`、`expression` など複数roleを分散して入れると、Machine Assistの見え方が安定します。
