...
派生Jobには `parent_job_id` が保存され、Dashboard、Job詳細、Compare画面から親Jobを確認できます。初回や新条件ではPilotを挟むと安全ですが、PreflightがOKで同条件の完走実績がある場合はStandardへ直行して構いません。
...
4. `SDXL 2D Face - Standard 6 Epoch`
   Dataset整備後の本番寄り短時間テストです。PreflightがOKで、同dataset version / base model / model familyの完走実績がある場合はPilotをスキップしてここへ直行できます。50枚前後のDatasetでは、おおよそ `50 images * repeats 2 * epochs 6 / batch 2 = 300 steps` を走らせ、epoch 4以降の過学習傾向、sample変化、採用候補epochを確認します。保存時メタデータ計算のメモリ負荷を避けるため `no_metadata` を使います。
