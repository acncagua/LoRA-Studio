...
評価入力は `evaluation_rubrics` の active schemaに沿って、rating 1〜5に加えて、強さ、過学習、採用判断、failure tagsを定型項目として保存します。memoは任意補足です。これにより、自由文の揺れに左右されず?? Engineが評価を使えます。将来ChatGPT画像評価を追加する場合も、このrubric schemaに従ったJSONを保存する想定です。現時点ではChatGPT API連携は行いません。
...
検証プリセットは、reForge / WebUIなど外部生成環境で人間が検証画像を作る時の標準条件です。LoRA-Studioは画像生成を自動実行せず、prompt、seed、weight、Hires条件を固定したプロンプトパックを出力し、生成後の画像を検証ランへ登録します。
