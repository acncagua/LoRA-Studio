...
Phase11.1時点では、Reference SetはMachine Review Assistの前段階です。embedding、CLIP/OpenCLIP/DINO/SigLIP、aesthetic score、reference similarity、prompt alignment、overfit risk、ChatGPT API、AI画像評価はまだ実装していません。画像とcaption snapshot、役割、メモを整理し、将来の機械レビューでも同じ基準を使えるようにするための管理機能です。
