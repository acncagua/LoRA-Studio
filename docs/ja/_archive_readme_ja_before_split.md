...
> このファイルはREADME_ja分割前の保全コピーです。内部リンクやスクリーンショット相対パスは、現行のREADME / docs構成と一致しない場合があります。
...
現在の運用ベータ記録は `CHANGELOG.md` の最新項目を参照してください。
...
LoRA-Studioに段階的なi18n基盤を追加しています。標準表示は日本語で、左ナビの言語切替、またはURLに `?lang=en` を付けることで英語表示にできます。READMEやOSS向けスクリーンショットを英語表示で撮れるようにするための基盤です。

初期対象は、左ナビ、ページ見出し、主要ボタン、状態バッジ、Primary Action、Recipe / Optimizer masterの表示名です。内部キーは英語識別子のまま扱います。sd-scripts log、生成command、raw args、tracebackは翻訳しません。

Built-in Recipe v2とOptimizer masterは、`labels_json`、`descriptions_json`、`risk_notes_json` に多言語表示を保存できます。Custom Recipeやユーザー入力名は翻訳せず、入力された名前をそのまま表示します。

UI翻訳辞書は `app/i18n/ja.json` と `app/i18n/en.json` に分離しています。新しい画面文言を追加する場合は、テンプレートへ直書きせず翻訳キーを追加して `t()` から表示してください。sd-scripts log、生成command、raw args、tracebackは翻訳対象外のままで問題ありません。

新しいAction stateには `label_key` と、必要に応じて `description_key` を持たせ、テンプレートではそのキーを優先して表示します。`ACTION_TEXT_KEYS` は既存の日本語action文言を壊さないための移行用fallbackであり、新規actionの主導線には使わない方針です。

## Phase 12.3.1: Practical Mini Pilot

Phase 12.3.1では、Phase 12.3のOptimizer Smoke Matrixの次段階として、Practical Mini Pilotを追加しました。

同時に、重い候補比較向けの中優先性能・I/O改善の土台を追加しています。

- Candidate Standard Comparisonでは、group内でweight 0 baseline画像を共有できます。Matrix上の論理条件は45枚×候補epochのまま、LoRAなしbaselineの物理生成だけを重複削減します。
- Candidate Comparison group単位でEmbeddingを実行できるようにし、共有baselineや同一パス画像を1回だけEmbedding対象にします。
- Runtime Storage Settingsを追加し、新規Jobのruns、Validation exports、logs、Embedding cacheをOneDrive外のruntime rootへ配置できるようにしました。既存DBに保存済みのartifact pathは移行せず、そのまま参照します。
- Performance Summaryでは、論理画像数、物理生成画像数、共有baseline数、生成/Embedding/Machine Review/Matrixの秒/枚を確認できます。

Smoke Testは起動確認です。Mini Pilotは100〜300step程度の短い実学習を行い、5000step級の本学習へ進む前に、明らかなOptimizer不適合を見つけるための軽量確認です。

Mini Pilotでは次を記録します。

- 短時間学習Jobとlog path
- loss summary、NaN / Inf検出
- LoRA artifact状態、safetensors読込確認
- sample画像数、image smoke状態
- optionalなMachine Review / mini validationの結果枠
- failure categoryと次に確認すること

Mini Pilot OKは品質保証ではありません。実用候補として次の評価へ進める合図です。
最終判断はCandidate Review / Weight Calibrationで行います。

## Phase 12.3: Optimizer Profile Validation

Phase 12.3では、Optimizer ProfileとRecipe v2に対して、sd-scriptsでPrepare / Smoke起動できるかを確認するための基盤を追加しました。
AdamW8bit以外のProdigy、Adafactor、Lion、DAdaptAdam、DAdaptLionなどを、マスタ登録だけでなく検証状態付きで扱えるようにするための機能です。

Optimizer Masterには、sd-scriptsへ渡す実行用パラメータをprofile単位で登録します。

- `sd_scripts_optimizer_type`
- Optimizerごとの必須パラメータ / 推奨パラメータ
- 通常Job作成で使うcommand params
- 2-step Smoke Testで使うsmoke params
- `bitsandbytes`、`prodigyopt`、`transformers`、`lion-pytorch`、`dadaptation` などの依存メモ

初期profileとして、AdamW8bit Balanced、PagedAdamW8bit Balanced、Prodigy Soft、
Adafactor Auto、Adafactor Fixed、Lion Soft、Lion Balanced Experimental、
DAdaptAdam Auto、DAdaptLion Autoを登録しています。

Optimizer詳細画面では次の操作ができます。

- Prepare Test Job: `command_argv.json`、dataset config、sample promptsの生成まで確認します。学習は実行しません。
- Run 2-step Smoke Test: 低dim・2stepだけの一時Jobを作り、sd-scriptsが起動できるかを確認します。
- Run Mini Pilot: 短時間の実学習確認Runを作成し、明示操作で実行できます。
- View Last Test Result: 最終結果、return code、command path、log pathを確認します。

Smoke Testは品質評価ではなく起動確認です。
`Smoke OK` は「現在の環境でコマンドが起動できた」ことを示すだけで、実用品質のLoRAになることは保証しません。
AdamW8bitを安定基準とし、それ以外のOptimizerはAdvanced / Experimentalとして、Smoke結果とログを確認しながら使う想定です。

Optimizerによってlearning rateの意味は違います。
AdamW8bit / PagedAdamW8bit / Lionは通常LRですが、Prodigy / DAdaptAdam / DAdaptLionの
`learning_rate=1.0` はAuto-LRへの倍率です。
Adafactor Autoは `relative_step=True` として `--learning_rate` / `--unet_lr` を出さず、
Adafactor Fixedは固定LRと `max_grad_norm=0.0` を使います。
...
`/jobs/new` は単一画面のまま、次の段階で設定を確認できます。

- 入口選択: 用途から選ぶ、Optimizerから選ぶ、既存Jobから派生、完全カスタム。
- Project / Dataset選択。
- Recipeカード選択。用途、Optimizer、Network Type、target steps、riskを見ながら選べます。
- Parameter Editor v2。Basic Params、Advanced Params、Raw Args、Resolved Params、User Override Diffを分けて表示します。
- Step Estimate、Target Step Assistant、Compatibility Checkを確認してから下書きJobを作成します。

Job作成時の上書きは `user_overrides_json` に `from` / `to` / `reason` 付きでsnapshot保存します。
Job詳細では、選んだRecipe、Optimizer、Network、Purpose、差分理由を確認できます。

Compatibility CheckはERROR / WARNING / NOTEを分けて表示します。
ERRORがある場合は下書きJob作成を止め、WARNINGは確認事項として表示します。
旧形式presetはlegacyとして折りたたみ表示に残し、既存Job互換も維持しています。

Recipe LibraryとOptimizer Masterはカード表示と詳細ページを追加しました。
実Jobの設定をCustom Recipeとして保存し、後続Jobで再利用するための最小導線も追加しています。

Phase 12.2.1ではRecipe表示名を整理しました。Model Family選択後は、Recipeカードのタイトルから
`SDXL` / `SD1.5` の繰り返しを省き、`顔キャラ・標準` のような短い名前を主表示にします。
検索や直接選択用にはModel Familyを含むfull labelを残しています。

- `short_label`: Recipeカードの短い主表示名です。
- `full_label`: 詳細・検索用の完全な表示名です。
- `direct_select_label`: 折りたたみ内の直接Recipe選択で使う表示名です。
- 用途から選ぶ場合は、Optimizerカテゴリ別にRecipeカードを並べます。
- Optimizerから選ぶ場合は、Purpose別にRecipeカードを並べます。

## Phase 12.1: Training Recipe / Optimizer Master v2

Phase 12.1では、Recipe v2とOptimizerを中心にした学習設定管理の土台を追加しました。
既存のlegacy presetと既存Jobはそのまま開けるようにしつつ、新規Job作成ではTraining Recipe、Optimizer Profile、Training Purpose、Network Typeを使う流れへ移行します。

Job作成には次の入口を用意しています。

- 用途から選ぶ: 顔LoRA、画風LoRA、衣装LoRA、物体、コンセプトなど目的からRecipeを選びます。
- Optimizerから選ぶ: AdamW8bit、Lion、Adafactor、DAdapt、ProdigyなどOptimizerを先に選びます。
- 既存Jobから派生: 過去JobのRecipe / params snapshotを引き継ぐ想定の導線です。
- 完全カスタム: Basic / Advanced / Raw Argsを手動調整します。

Optimizerごとにlearning rateの意味は異なります。
AdamW / Lionは通常LRですが、DAdapt / Prodigyの `learning_rate=1.0` は自動LRへの倍率であり、Adafactorはrelative_step運用か固定LR運用かで意味が変わります。

target stepsはRecipe、Optimizer Profile、Optimizer Definition、global fallbackの順で解決します。
expected stepsは次の式で計算します。
...
通常運用では `max_train_steps` を直接指定せず、Target Step Assistantで `repeats` / `epochs` / `batch` を調整します。
`max_train_steps` と Raw Args はAdvanced扱いです。sd-scriptsの引数仕様を理解している場合のみ使ってください。

LoCon / LoHa / LoKr / LyCORISなどのNetwork TypeはPhase 12.1ではmetadataとして表示し、実行対象はStandard LoRA中心です。本格対応はPhase 12.4以降の予定です。

## Phase 11.8: Post-training Review Automation

Post-training Review Automationは、Training Job完了後に、ユーザーが手動でReview Sessionを作る前段階を自動化します。
loss候補epochを抽出し、採用前Review Planを作成します。設定によっては、軽量なQuick Candidate Review Matrixの準備まで自動開始できます。

modeは次の4種類です。

- `manual`: 従来通り、Review Session作成と実行を人間が行います。
- `plan_only`: Training Job完了後にReview Planだけ作成します。画像生成は開始しません。defaultです。
- `quick_auto`: 安全条件を満たす場合、軽量候補レビューを自動開始します。
- `standard_auto`: 候補epochごとに標準45条件相当で確認するため、実行時間とGPU負荷が増える可能性があります。通常は `max_auto_images` 超過時に確認待ちで停止します。

Quick Candidate Reviewは採用前のepoch比較用です。
候補epochは最大3件、promptは `basic_face` / `full_body` / `expression_pose`、seedは `111111`、weightは `0.6` / `0.8`、Hiresなし、最大18枚を想定します。
採用後にweightを詰めるWeight Calibrationとは別工程で、標準45枚検証の代わりではありません。

Machine Assistのスコア差が小さい場合は、無理に「有力候補」を1つに決めず、`no_clear_winner` の候補群として表示します。
これはReference similarity差が小さい、provider間順位が一致しない、人間評価が未入力などの場合に、人間評価を優先するための表示です。
Machine Assistは最終判断を置き換えず、採用やProfile反映を自動実行しません。

## Candidate Review Modes

LoRA-Studioでは、採用前のepoch比較と採用後のweight検証を分けています。

- Quick Candidate Review: 採用epochを決めるための軽量レビューです。
- Standard Candidate Comparison: loss候補epochの `primary` / `secondary` / `check` それぞれにStandard Validation v1を作成し、45枚 x 候補epoch数を1つのcomparison groupとして一括実行します。最後にEpoch横断Matrixで比較できます。
- Manual: 特殊なepochやpresetを使う場合の従来の手動Validation Run導線です。
- Weight Calibration: 採用済みLoRA / selected outputに対して、推奨weight範囲を決める採用後検証です。

Standard Candidate Comparisonは、Quick Candidate Reviewだけでは判断しづらい場合に、候補epochを同じ45枚条件で重めに比較するための導線です。
開始前に候補epoch、画像数、推定時間、推定容量を表示します。

## Phase 11.9: Retry Signal Summary

Retry Signal Summaryは、自動リトライ本体の前段階として、結果を読み取り専用で分類表示するためのサマリです。
Training / Review / Weight Calibrationの結果から、リトライが必要そうか、または人間が次に何を確認するべきかを表示します。

入力として、expected steps vs target、loss trend、best candidate epoch位置、Review SessionのMachine Assist、人間評価、Weight Calibration結果、推奨weight範囲、overfit risk、failure tagsを参照します。
...
同期対象配下では、ファイルロック、メタデータ走査、アップロード負荷が、sd-scripts画像生成、Embedding、Machine Reviewの読み書きと競合することがあります。
Performance Summaryでは、`runs`、`exports`、`data/embeddings`、model pathがOneDrive配下に見える場合にWARNINGを表示します。

## Phase 11.7: Weight Calibration Pipeline

Candidate ReviewとWeight Calibrationは別の工程です。
Candidate Reviewは「どのepochを採用するか」を決める採用前レビューです。
Weight Calibrationは採用済みLoRA / selected outputに対して、「何倍で使うか」を決める採用後weight検証です。

Standard Validationは45枚です。
内訳は `3 prompts x 3 seeds x 5 weights` で、weight `0` はbaselineとしてLoRAタグ / network weightsなしで生成します。
標準比較はHiresなしを基準にし、Extended ValidationのHiresあり/なし比較は最終見栄え確認として扱います。

Weight Calibration Pipelineは、Project / Job / Review Session / LoRA Profile / Validation Runの導線から開始できます。
PipelineはExpected Conditions確認、sd-scripts画像生成、生成画像DB登録、Embedding計算、Machine Review Assist、Weight Review Matrix生成を順番に実行します。
Machine Assistは補助情報であり、推奨weightでは人間評価を優先します。
Suggested Weightは自動反映せず、Validation Run詳細の「Profileへ反映」で人間が明示的にApplyします。

## Step Estimator / Target Step Assistant

epoch数だけでは学習量は判断できません。
LoRA-Studioでは、以下の式でexpected stepsを見積もります。
...
Job作成、Job編集、Job詳細、Preflightで、Optimizer target / Recipe targetと比較して表示します。
target stepsは `training_recipes`、`optimizer_profiles`、`optimizer_definitions` の順に参照し、
未設定時はglobal fallbackを使います。
Target Step Assistantでは、Recipe target recommendedを初期値にしてrepeatsを自動計算し、
epoch数が多い場合はsave/sample intervalも提案します。
repeatsを増やしても出力LoRA数は増えませんが、1epochあたりのstep数が増えます。
repeatsが大きすぎる場合は、同じ画像を見すぎるため過学習や固定化に注意してください。

`max_train_steps` の直接指定はAdvanced扱いです。
epoch基準レビューとの対応が分かりにくくなるため、通常はrepeats / epochs / batchで調整する運用を推奨します。
...
- `クイック検証 v1`: 3 prompts × 2 seeds × 3 weights × Hiresなし = 18枚。`0.6 / 0.8 / 1.0` の実用寄りweightを短時間で確認します。
- `標準検証 v1`: 3 prompts × 3 seeds × 5 weights × Hiresなし = 45枚。採用候補LoRAの標準比較です。推奨weightは原則としてこのHiresなし結果を基準にします。
- `拡張検証 v1`: 3 prompts × 3 seeds × 5 weights × Hiresあり/なし = 90枚。標準検証にHiresあり条件を加えた最終見栄え確認です。

標準比較はHiresなしで行います。Hiresありは最終見栄え確認であり、Hiresなしの素のLoRA比較とは直接混ぜて判断しません。weightを毎回細かく刻みすぎると生成時間が増えるため、まずはクイック検証、採用候補なら標準検証、最後に拡張検証の順で確認します。
...
- `validation_prompts_for_sd_scripts.txt`: 検証条件をまとめた確認用prompt file。
...
Hiresあり条件はsd-scriptsのHighres fixで生成します。WebUI / reForgeのHires.fixやDPM++系サンプラーと完全一致するものではなく、LoRA-Studio内で同条件を横並び比較するための目安です。最終運用確認は必要に応じてreForge / WebUIでも少数行ってください。
...
- `transformers_clip`: Phase 11.4で追加した初回の実embedding providerです。標準モデルは `openai/clip-vit-base-patch32`、出力は512次元です。`transformers` と `torch` を使い、deviceはcuda優先、使えない場合はcpuへfallbackします。fp16はcuda時のみ使い、cpuではfp32になります。
- `open_clip`: Phase 11.5で試験導入した実embedding providerです。標準モデルは `ViT-B-32` / `laion2b_s34b_b79k`、出力は512次元です。`open_clip_torch` と `torch` を使い、deviceはcuda優先、使えない場合はcpuへfallbackします。fp16はcuda時のみ使い、cpuではfp32になります。

Embedding設定は `Embedding設定` 画面で管理します。初期値は `mock_image_512`、cache rootは `data/embeddings`、自動downloadはOFFです。`transformers_clip` と `open_clip` は `モデルdownloadを許可` がONの場合だけモデルweightのdownloadを許可します。OFFのまま実行する場合は、モデルがローカルcacheに存在している必要があります。

GPUを使う実providerは同時1件だけ実行します。学習ジョブやsd-scripts画像生成がrunningの場合、Embedding Jobは開始しません。mock providerはCI/テスト用として残しており、外部モデルdownloadやGPUは不要です。
...
`transformers_clip` / `open_clip` providerは、画像全体の意味・構図・雰囲気を拾いやすい一方、顔専用の同一性判定ではありません。顔の細部、衣装の細かい差、キャラクター固有パーツは人間評価を優先してください。特にfull body画像は顔が小さくなるため、顔類似度として過信しないでください。Reference Setには `face_front`、`upper_body`、`full_body`、`expression` など複数roleを分散して入れると、Machine Assistの見え方が安定します。
...
Post-training Review Automationを有効にすると、Training Jobが `completed` になった時点でこの採用前Review Planを自動作成できます。
defaultの `plan_only` はReview Planだけを作成し、画像生成は人間が開始します。
`quick_auto` は、Training / Validation / Embedding / Machine ReviewなどのGPUタスクが走っておらず、expected image countが `max_auto_images` 以内の場合だけ軽量候補レビューを自動開始します。
...
Machine Assistのスコアが僅差の場合は、MatrixとReview Session詳細で `no_clear_winner` と候補群を表示します。
この場合は機械スコアを採用判断として扱わず、画像比較と人間評価を優先してください。
候補epochの周辺をもう少し確認したい場合は、Review Session詳細の「近隣Epochを追加検証」を使います。
中心epochと±1/±2を選ぶと、同じTraining Jobと親Review Sessionに紐づいたplanned状態の追加Review Sessionを作成します。
条件はweight `0.6` / `0.8`、seed `111111`、Hiresなしです。
